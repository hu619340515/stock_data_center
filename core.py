...
def worker_download(process_id: int, stock_list: pd.DataFrame, start_date: str, end_date: str, result_queue: Queue, frequency: str = "d") -> None:
    """
    🏭 子进程：负责下载 + 实时打印进度
...
class StockDataPipeline:
    ...
    def daily_update_pipeline(self, frequency: str = "d") -> None:
        ...
        safe_print(f"🔄 启动增量更新流水线 (频率: {frequency})...")
        safe_print(f"⏰ 开始时间: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        safe_print(f"📋 待处理股票数量: {total_stocks} 只")
        ...
        for i, (_, row) in enumerate(stocks.iterrows()):
            ...
            if start_date <= end_date:
                try:
                    df = self.current_data_source.get_stock_history(code, start_date, end_date, frequency)
                    if not df.empty:
                        batch_buffer.append(df)
                        safe_print(f"✅ 增量更新: {code}")
                        success_count += 1
                    else:
                        safe_print(f"ℹ️ {code} 无新数据")
                except Exception as e:
                    safe_print(f"❌ {code} 更新失败: {e}")
                    error_count += 1
                    # 尝试切换数据源
                    if error_count % 5 == 0:  # 每5个错误尝试切换一次
                        self._switch_data_source()
            else:
                safe_print(f"ℹ️ {code} 数据已最新")
            ...
            if (i + 1) % 10 == 0 or (i + 1) == total_stocks:
                ...
                if elapsed_time > 0:
                    speed = (i + 1) / elapsed_time  # 股票/秒
                    estimated_total = total_stocks / speed if speed > 0 else 0
                    remaining = estimated_total - elapsed_time
                    
                    progress_percent = ((i + 1) / total_stocks) * 100
                    safe_print(f"📊 进度: {i + 1}/{total_stocks} ({progress_percent:.1f}%) | 速度: {speed:.2f} 只/秒 | 剩余: {remaining:.0f} 秒")

            if len(batch_buffer) >= 20:
                self.db.upload_batch(batch_buffer, frequency)
                batch_buffer = []
                safe_print(f"💾 完成一批入库 (批大小: 20, 频率: {frequency})")
        ...
        # 显示统计信息
        safe_print(f"\n📋 任务统计")
        safe_print(f"✅ 成功: {success_count} 只")
        safe_print(f"❌ 失败: {error_count} 只")
        safe_print(f"📊 总处理: {total_count} 只")
        safe_print(f"⏰ 耗时: {elapsed_time:.2f} 秒")
        if elapsed_time > 0:
            safe_print(f"⚡ 平均速度: {total_count / elapsed_time:.2f} 只/秒")
        safe_print(f"📈 成功率: {(success_count / total_count * 100):.1f}%" if total_count > 0 else "📈 成功率: 0%")
